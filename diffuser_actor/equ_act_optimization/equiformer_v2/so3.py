...
class SO3_FiLM_Linear(torch.nn.Module):
    def __init__(self, in_features, out_features, lmax, condition_dim):
        '''
        SO3 Equivariant FiLM layer
        FiLM modulation https://arxiv.org/abs/1709.07871
        predicts the  scale and bias for per-channel in the nn and per-type of spherical harmonics
            1. Use `torch.einsum` to prevent slicing and concatenation
            2. Need to specify some behaviors in `no_weight_decay` and weight initialization.
        '''
        super().__init__()
        ...
        self.out_features = out_features
        self.lmax = lmax
        self.film_weight_kaiming_var = torch.sqrt(torch.tensor(self.lmax * self.in_features)).item()
        weight_dim, bias_dim = (self.lmax + 1) * out_features * in_features, out_features
        self.cond_encoder = nn.Sequential(
            nn.Mish(),
            nn.Linear(condition_dim, weight_dim+bias_dim),
        )
    ...
    def forward(self, input_embedding, condition):
        film_param = self.cond_encoder(condition)
        weight, bias = film_param[:, :-self.out_features], film_param[:, -self.out_features:]
        # Kaiming initialization https://arxiv.org/pdf/1502.01852v1
        weight = weight / self.film_weight_kaiming_var
        bias = bias / 10
        weight = weight.reshape((-1, self.lmax+1, self.out_features, self.in_features))  # ZXP: FiLM initialization
        # [b, 1, (L_max + 1) ** 2, C_out, C_in]
        weight = torch.index_select(weight, dim=1, index=self.expand_index).unsqueeze(1)
        batch_size = film_param.shape[0]
        out = torch.einsum('bnmi, bemoi -> bnmo',  # ZXP ToDo: check the einsum
                           input_embedding.embedding.reshape(batch_size, -1, (self.lmax+1)**2, self.in_features),
                           weight)  # [b, N, (L_max + 1) ** 2, C_out]
        bias = bias.view(batch_size, 1, 1, self.out_features)
        out[:, :, 0:1, :] = out.narrow(2, 0, 1) + bias
        out = einops.rearrange(out, 'b n m o -> (b n) m o')  # [(b N), (L_max + 1) ** 2, C_out]
        out_embedding = SO3_Embedding(
            0,
            input_embedding.lmax_list.copy(),
            self.out_features,
